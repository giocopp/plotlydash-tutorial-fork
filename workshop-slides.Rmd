...
.details[.code120[
```{r, eval=TRUE, message=FALSE, warning=FALSE, echo=TRUE, fig.width=0.5, fig.height=0.2}
fig1b <- beers %>% 
...
.details[.code120[
```{r, eval=TRUE, message=FALSE, warning=FALSE, echo=F, fig.width=1.85, fig.height=1.15}
fig1b
```
]]
</details>]
...
Now, let's compare the histogram we created previously with another showing the distribution of beer production in terms of IBU level.
<br>
...
fig1c <- beers %>% 
  plot_ly(x = ~ibu, type = "histogram", name = "IBU") %>% 
          layout(
    title = "IBU Histogram",
    xaxis = list(title = "International Bitterness Units (IBU)"),
    yaxis = list(title = "Count of beers"),
    bargap=0.2
  )
